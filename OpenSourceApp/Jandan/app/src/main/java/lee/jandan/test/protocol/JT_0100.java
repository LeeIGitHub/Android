...
import lee.jandan.test.T808Message.MyBuffer;
    ...
    public final void ReadFromBytes(byte[] bytes) {
        ...
        if (BitConverter.IsLittleEndian) {
            ...
            if (bytes.length < 36) {
                ...
                setLicenseNo(buff.getString(bytes.length - 25));
            } else {
                // 808补充协议 终端型号是20个字节
                setTerminalModelNo(buff.getString(20));
                setTerminalId(buff.getString(7));
                setPlateColor(buff.get());
                setLicenseNo(buff.getString(bytes.length - 37));
            }
            ...
            this.licenseNo = buff.getString(bytes.length - dataLen);
