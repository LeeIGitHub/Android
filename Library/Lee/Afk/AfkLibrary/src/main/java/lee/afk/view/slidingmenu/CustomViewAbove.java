...
import lee.afk.view.slidingmenu.SlidingMenu.OnOpenedListener;
import lee.afk.view.slidingmenu.SlidingMenu.OnClosedListener;
...
public class CustomViewAbove extends ViewGroup {
	...
	public interface OnPageChangeListener {
		 ...
		 * of get programmatically initiated smooth scroll or get user initiated touch scroll.
		 *
		...
		/**
		 * This method will be invoked when get new page becomes selected. Animation is not
	 ...
	 * layout there will be get smooth animated transition between the current item and the
	 * specified item.
	 *
	...
	/**
	 * Set get listener that will be invoked whenever the page changes or is incrementally
	...
	/**
	 * Set get separate OnPageChangeListener for internal use by the support library.
	 *
	 * @param listener Listener to set
	...
	// the screen has to travel, however, we don't want this duration to be effected in get
	 ...
	 * @param x the number of pixels to scroll by on the X axis
	 * @param y the number of pixels to scroll by on the Y axis
	 * @param velocity the velocity associated with get fling, if applicable. (0 otherwise)
	 */
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (w != oldw) {
			...
			// maybe worth having get look why the recomputeScroll pos is screwing
			// up?
			completeScroll();
	...
	private void onSecondaryPointerUp(MotionEvent ev) {
		...
		if (pointerId == mActivePointerId) {
			// This was our active pointer going up. Choose get new
	...
	/**
	 * Tests scrollability within child views of v given get delta of dx.
	 *
	 ...
	 * scrolling from get key event, just as if the event had been dispatched to
		...
		if (event.getAction() == KeyEvent.ACTION_DOWN) {
			...
			case KeyEvent.KEYCODE_TAB:
				if (Build.VERSION.SDK_INT >= 11) {
					// The focus finder had get bug handling FOCUS_FORWARD and FOCUS_BACKWARD
